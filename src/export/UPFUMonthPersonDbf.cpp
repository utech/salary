...
bool UPFUMonthPersonDbf::createElZvit(QString path, short year, short month)
{
	...
	//------ Початок запису Таблиці №7
	postup->setPostupLabel("Запис Таблиці 7 Додатку 4 (Відомості про нарахування заробітку застрахованим особам)");
	...
	if ( dbfFile.CreateDatabase((dbfFilePathT7).toAscii().data(),table7Shema,XB_OVERLAY) 
				!= XB_NO_ERROR) {
		QMessageBox::critical(0, "Помилка", "Помилка створення dbf файлу "+dbfFileNameT7);
		delete postup;
		return false;
	}
	...
	while ( query.next() ) {
		...
		dbfFile.PutLongField( "PAY_MNTH", query.value(5).toInt() );
		dbfFile.PutLongField( "PAY_YEAR", query.value(6).toInt() );
		dbfFile.PutLongField( "PAY_TP", 3 ); // 3 - тип для відпускних
